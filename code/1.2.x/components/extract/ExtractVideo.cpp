...
ExtractVideo::ExtractVideo(const char *srcFilePath, const char *dstFilePath) : ExtractBase(srcFilePath)
{
	...
	if (deCodeContPointer->GetResult() < 0)
		goto end;

	enCodeContPointer = new EnCodecVideoContext(EncodeName::LIBX264,
	...
	if (enCodeContPointer->GetResult() < 0)
		goto end;
	...
	if (outFmtContPointer->GetResult() < 0)
		goto end;
	ret = 0;
	return;
end:
...
void ExtractVideo::DoExtract()
{
	...
	while ((deFrame = deCodeContPointer->GetNextFrame(*inFmtContPointer)) != nullptr)
	{
		...
		{
			// rescale
			if (swsContPointer->RescaleVideoFrame(deFrame, *enCodeContPointer) == false)
				continue;
			// encode
