...
AVSwrContext::AVSwrContext(AVCodecContext *deAudioCodecCont,
						   int enSampleRate, AVSampleFormat enSampleFormat, AVChannelLayout enChLayout) : AVSwrContext(deAudioCodecCont->sample_rate, deAudioCodecCont->sample_fmt, deAudioCodecCont->ch_layout,
																													   enSampleRate, enSampleFormat, enChLayout)
{
}

AVSwrContext::AVSwrContext(int deSampleRate, AVSampleFormat deSampleFormat, AVChannelLayout deChLayout,
						   AVCodecContext *enAudioCodecCont) : AVSwrContext(deSampleRate, deSampleFormat, deChLayout,
																			enAudioCodecCont->sample_rate, enAudioCodecCont->sample_fmt, enAudioCodecCont->ch_layout)
{
}

AVSwrContext::AVSwrContext(AVCodecContext *deAudioCodecCont, AVCodecContext *enAudioCodecCont) : AVSwrContext(deAudioCodecCont->sample_rate, deAudioCodecCont->sample_fmt, deAudioCodecCont->ch_layout,
																											  enAudioCodecCont->sample_rate, enAudioCodecCont->sample_fmt, enAudioCodecCont->ch_layout)
{
}
...
bool AVSwrContext::ResampleAudioFrame(AVFrame *deFrame, AVFrame *&enFrame)
{
	...
	// av_log_pframe("resample success,EnAudio Frame sample number:%d\n", ret);
	if (ret < 0)
	{
