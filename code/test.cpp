...
Video::Video(const Prefs &prefsParam, const QString &filenameParam, const bool enableCache) : filename(filenameParam)
{
    ...
    if (_prefs._mainwPtr)
    { // during testing, mainPtr is not always defined, giving warnings : here we supress them !
...
void Video::run()
{
    ...
    if (_useCacheDb && cache.readMetadata(*this))
    ...
    if (ret == _failure)
    {
        ...
        emit rejectVideo(this, "failed to take capture");
             ...
             (_prefs._thumbnails == cutEnds && hash[0] == 0 && hash[1] == 0))
...
int Video::takeScreenCaptures(const Db &cache)
{
    ...
    while (--capture >= 0) // screen captures are taken in reverse order so errors are found early
    {
        ...
        { // metadata parsing error or variable resolution
...
QImage Video::ffmpegLib_captureAt(const int percent, const int ofDuration)
{
    ...
    qDebug() << "avg fps=" << (float)vs->avg_frame_rate.num / vs->avg_frame_rate.den << " stream bitrate= " << vs->codecpar->bit_rate / 1000 << "kb/s"
    ...
    qDebug() << "Seeking to timestamp " << msToHHMMSS(ms_stream_duration * (percent * ofDuration) / (100 * 100)) << " for file (of duration " << msToHHMMSS(ms_stream_duration) << " ) ";
    ...
    while (readFrame == false)
    {
        ...
        if (vPacket->stream_index == stream_index)
        {
            ...
            { // should debug codes, 0 means success, otherwise just try again
                ...
                qDebug() << "Frame "
                         << "type " << av_get_picture_type_char(vFrame->pict_type) << " frame number " << codec_ctx->frame_number;
                qDebug() << " wanted ts (from start)" << wanted_ts << " in time (from 0)" << msToHHMMSS(1000 * (wanted_ts - start_time) * vs->time_base.num / vs->time_base.den) << " vFrame pts " << vFrame->pts << " in time (from0)" << msToHHMMSS(1000 * (vFrame->pts - start_time) * vs->time_base.num / vs->time_base.den) << " pkt dts " << vFrame->pkt_dts << " curr_ts " << curr_ts << " best effort ts " << vFrame->best_effort_timestamp;
#endif
                ...
                if (frames_estimated != -1 && frames_estimated < ESTIMATE_DURATION_FROM_FRAME_NB && vFrame->pkt_size != 0 && vs->time_base.num != 0 /*avoid 0 divisions*/)
                {
                    ...
                    qDebug() << "No video duration : trying to estimate from file size and frame size : "
                             << " frame pkt size " << vFrame->pkt_size << " avg " << frames_estimated << " frames size " << avg_frame_bytes_size << " for file size (in bytes)" << size << " making estimated nb frames " << size / avg_frame_bytes_size << " or estimated duration " << msToHHMMSS(1000 * vs->avg_frame_rate.den * size / (avg_frame_bytes_size * vs->avg_frame_rate.num));
#endif
                ...
                if ((curr_ts - wanted_ts) >= -ts_diff)
                { // must read frames until we get before wanted time stamp
