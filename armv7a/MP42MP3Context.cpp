...
void MyLogOutput(void *ptr, int level, const char *fmt, va_list vl)
{
    FILE *fp = fopen("/storage/emulated/0/aaatest/logg.txt", "a+");
    if (fp)
    {
        vfprintf(fp, fmt, vl);
        fflush(fp);
...
void MP42MP3Context::TempDecodePacket(void (MP42MP3Context::*funcP)())
{
    ...
    av_log_info("packet numbers:{0}", packetCount);
    const char *fmt;
