...
VAContextFromUnity::VAContextFromUnity(const char* srcFilePath) : ContextBase(srcFilePath), videoPacketCount(0), audioPacketCount(0)
{

}
VAContextFromUnity::VAContextFromUnity(const char* srcFilePath, const char* dstFilepath) : VAContextFromUnity(srcFilePath)
{
...
VAContextFromUnity::VAContextFromUnity(const char* srcFilePath, const char* dstFilepath, int fps, int width, int height) : VAContextFromUnity(srcFilePath)
{
    SetAudioOutputStreamStruct();
...
void VAContextFromUnity::MuxeVideoAndAudio(const char* dstFilepath)
{
    if (dePacket == nullptr)
...
void VAContextFromUnity::WriteToFile()
{
    for (AVPacket* packet : audioPackets)
    {
        av_interleaved_write_frame(outFmtCtx, packet);
        //av_log_info("audio packet:%p\n", packet);
        av_packet_free(&packet);
    }
    for (AVPacket* packet : videoPackets)
    {
        av_interleaved_write_frame(outFmtCtx, packet);
        //av_log_info("video packet:%p\n", packet);
        av_packet_free(&packet);
    }
}
void VAContextFromUnity::InitOutFormatContextByFileName(const char* dstFilePath)
{
...
void VAContextFromUnity::SetAudioOutputStreamStruct()
{
    int audioStreamIndex = av_find_best_stream(fmtCtx, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
...
void VAContextFromUnity::RescaleDeVideoFrame()
{
...
void VAContextFromUnity::ResampleDeAudioFrame()
{
...
void VAContextFromUnity::EncodeAudioFrame()
{
    if (audioSt.frame != nullptr) {
        ...
        audioSt.next_pts += audioSt.frame->nb_samples;
    }
...
void VAContextFromUnity::OutputStreamFree(OutputStream& ost) {
...
void VAContextFromUnity::SetVideoOutputStreamStruct()
{
    int videoStreamIndex = av_find_best_stream(fmtCtx, AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
    AVCodecContext* videoDenc = OpenDecodecContextByStreamPar(fmtCtx->streams[videoStreamIndex]);
    videoSt.denc = videoDenc;
    int fps = av_q2d(fmtCtx->streams[videoStreamIndex]->r_frame_rate);
    videoSt.enc = OpenVideoEncodecContext(AV_CODEC_ID_H264, fps, videoDenc->width, videoDenc->height); // error prone
...
void VAContextFromUnity::SetVideoOutputStreamStruct(int fps, int width, int height)
{
    int videoStreamIndex = av_find_best_stream(fmtCtx, AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
    AVCodecContext* videoDenc = OpenDecodecContextByStreamPar(fmtCtx->streams[videoStreamIndex]);
    videoSt.denc = videoDenc;
